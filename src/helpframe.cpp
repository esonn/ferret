...
HelpFrame::HelpFrame ()
	: wxDialog (NULL, wxID_ANY, wxT("Ferret: Help"),
...
"More information about Ferret, and the Plagiarism Detection Group, can be found at our website: http://homepages.feis.herts.ac.uk/~pdgroup.\n\n"
"IMPORTANT DISCLAIMER: This software comes with ABSOLUTELY NO WARRANTY; users of this software do so at their own risk.  Neither the University of Hertfordshire nor the individuals involved in any part of this software will accept any liability for any damage or harm caused to the data, computer or computer software by using this software.\n");
	...
	wxString credits_text = wxT("The original concept of using trigrams for measuring copying was developed by Caroline Lyon and James Malcolm.  JunPeng Bao, Ruth Barrett and Bob Dickerson also contributed to the development of earlier versions of Ferret.\n\n" 
...
"Ferret is capable of analysing two types of documents: natural language, where tokens are typically taken as words; and computer programs, where tokens are symbols within the language.  Ferret will determine the type based on the first listed document's filename.  If the choice is wrong, because, for example, computer programs have been saved with a 'txt' extension, then the type may be changed by clicking on the appropriate choice.  Note, Ferret's choice will be correct in most cases, but if you want to change it, this must be done after adding all the documents for comparison.\n\n" 
"When converting files, Ferret will save the files into the named folder.  The user can direct Ferret to use a different folder using the 'Browse ...' button.  This option is particularly useful for collecting together files from multiple folders into one place.\nFerret will only copy the converted files to the destination folder; check the first box if you want every file to be copied.\n\nIf you want every file to be treated as a word/rtf file (for example, with mislabelled student or pupil work), check the second box.\n\nIf you want to ignore any unknown files (for example, output from compilers or image files), check the third box.  (Ferret's initial settings mean it will ignore unknown filetypes.)\n\n" 
"Ferret will treat files ending in .doc or .rtf as word/rtf files, and use 'abiword' to convert them to text.  Files ending in .pdf will be converted using 'pdftotext'.  Files ending in .txt will be treated as plain text files, and not converted.  Files ending in .cpp, .c, .java or .h will be treated as computer programs, and not converted.\n\n");
	
	wxString table_text = wxT("Ferret produces a similarity score for every pair of documents in the selected list.  The similarity score runs from 0 (no copied sequences) to 1 (every sequence is copied).  Initially, the table is arranged with the most similar pairs of documents at the top.\n\nThe table may be rearranged into alphabetical order of name or numerical order of similarity by clicking on the respective button to the right (clicking on the column heading also rearranges the table).  The table, along with other relevant information, may be saved as a pdf file, using the 'Save Report ...' button; the user will be prompted for a folder and filename to save the report in.\n\n" "You can see details of how the two documents are analysed, and the copied sequences, by clicking on a line in the table.  The 'Show Analysis' button will open a new window, showing the two documents; you can also show the analysis by double-clicking on the line.  The 'Save Analysis ...' button creates a pdf report highlighting the common trigrams in the two documents; the user will be prompted for a folder and filename to save the analysis in.\n\nYou may open as many of the analysis windows as you wish.\n");

	wxString comparison_text = wxT("The chosen documents are shown with common trigrams highlighted in blue.\n\nThe list on the right shows all of the common trigrams contained in these two documents.  Click on a trigram in the list to highlight every occurrence of that trigram in the documents.  The 'Find' buttons below each document's display will allow you to step through every occurrence of the trigram in that document.\n\nYou may save a pdf report of the comparison by clicking the 'Save Analysis' button on the lower right; the report will highlight the common trigrams.\n");

	// -- add pages
	notebook->AddPage (MakePage (notebook, wxGetApp().GetVersionString () + wxT("\n\n"), about_text), 
			wxT("About"), true);
	notebook->AddPage (MakePage (notebook, wxT("How Ferret was Created\n\n"), credits_text), 
			wxT("Credits"), false);
	notebook->AddPage (MakePage (notebook, wxT("Selecting the Documents to Compare\n\n"), selection_text),
			wxT("Select"), false);
	notebook->AddPage (MakePage (notebook, wxT("Ferret's Table of Comparisons\n\n"), table_text),
			wxT("Compare"), false);
	notebook->AddPage (MakePage (notebook, wxT("Analysis Window\n\n"), comparison_text),
			wxT("Analyse"), false);
