...
"Since version 3.0, Ferret has been rewritten using the wxWidgets cross-platform library - Peter Lane has produced the current implementation.\n\nText conversion is achieved by calling abiword: http://www.abisource.com, and pdftotext: http://www.xpdf.com";
	...
	wxString selection_text = "The first step in running Ferret is to identify the documents to be compared.  Add the filenames or directories of documents to the 'analysis' list by using the 'Add Documents ...' or 'Add Directory ...' button, and choosing the filenames to compare.  You can repeat the operation to add documents from different folders.  (If all your files are in a single folder, you can usually open that folder, select the first item, and then press CONTROL and A together to select all the files.)  You can also drag and drop files from your file manager into the list.\n\n"

    "Use the 'analysis' area for files to compare for similarity, and the 'template' area for files whose contents you want to exclude from analysis.\n\n"
