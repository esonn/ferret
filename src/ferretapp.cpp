...
FerretApp::FerretApp ()
{
	...
	// create the folder for downloading files to 
	_download_folder = wxFileName::GetCwd () + wxFILE_SEP_PATH + wxT("FerretDownloadFolder");
	if (!wxFileName::DirExists (_download_folder))
		wxFileName::Mkdir (_download_folder);
	...
	_max_downloaded_documents = 10;
	_max_results_per_tuple = 10;
	_max_tuple_searches = 10;
	_last_x = 0;
	_last_y = (
	#if __WXMAC__ 
	22
...
// TODO: There is an inherent problem in locating the download-files.rb file
// Use platform specific filenames and installation techniques
wxString FerretApp::GetDownloadFile () const
{
#if __WXMSW__
...
wxString FerretApp::GetDownloadFolder () const
{
	return _download_folder;
}

void FerretApp::SetDownloadFolder (wxString download_folder)
{
...
int FerretApp::GetMaxDownloadedDocuments () const
{
	return _max_downloaded_documents;
}

int FerretApp::GetMaxResultsPerTuple () const
{
	return _max_results_per_tuple;
}

int FerretApp::GetMaxTupleSearches () const
{
	return _max_tuple_searches;
}

void FerretApp::SetMaxDownloadedDocuments (int val)
{
	_max_downloaded_documents = val;
}

void FerretApp::SetMaxResultsPerTuple (int val)
{
	_max_results_per_tuple = val;
}

void FerretApp::SetMaxTupleSearches (int val)
{
	_max_tuple_searches = val;
}
